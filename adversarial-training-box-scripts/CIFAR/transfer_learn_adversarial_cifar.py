...
if __name__ == "__main__":
    ...
    training_stack = []
    training_stack.append((30, StandardTrainingModule(criterion=criterion, attack=PGDAttack(epsilon_step_size=8/255/mean_std/4, number_iterations=7, random_init=True), epsilon=training_parameters.attack_epsilon/mean_std)))
    ...
    experiment_tracker.initialize_new_experiment(f"TL{retraining_layers}_AdversarialB128", training_parameters=training_parameters | training_objects)
    pipeline = Pipeline(experiment_tracker, training_parameters, criterion, optimizer, scheduler)
    
    # Train
    pipeline.train(train_loader, converted_model, training_stack, early_stopper=None, 
